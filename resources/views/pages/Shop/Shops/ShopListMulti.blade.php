...
<div class="card">
    <form id="list_data"  method="get" action="{{route('index_Add_multi') }}">
        ...
        <div class="row">
            ...
            <div class="col-md-1 mb-3 text-left">
                <div class="generateshops">
                    <button style="margin-top: 5px;" type="button" onclick="reset_form('list_data')"  class="btn btn-danger btn-xs">Clear</button>
                </div>
            </div>
          </div>
        </div>
   ...
   {{-- @include('pages.Shop.Shops.ShopListAjaxMulti') --}}
  ...
  
{
    ...
    render: function (data) {
        if (data) {
            const url = APP_URL + '/public/storage/shop_image/' + data;
            return `<a href="${url}" target="_blank">
                        <img src="${url}" class="datatable-img" />
                    </a>`;
        }
        return 'No Image';
    }
            ...
            data_table().initialize('{{ route("index_Add_multi") }}', columns, filterInputs);
