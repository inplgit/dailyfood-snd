            ...
            @foreach ($result as $item)
                @php
                    ...
                    if(!empty($from)){
                        ...
                        $get_purchase_previous = $preparedPurchaseDataPrevious[$item->product_id][$item->flavour_id] ?? ['main_qty' => '', 'main_amount' => 0];
                        ...
                        $get_opening_previous = $preparedData[$item->product_id][$item->flavour_id] ?? ['main_qty' => '', 'main_amount' => 0];
                        ...
                        $get_sales_return_previous = $preparedSalesReturnDataPrevious[$item->product_id][$item->flavour_id] ?? ['main_qty' => '', 'main_amount' => 0];
                    ...
                    $total_return_value_previous += (float)$return_amount_previous;
                        ...
                        $get_transfer_received_previous = $preparedTransferRecievedDataPrevious[$item->product_id][$item->flavour_id] ?? ['main_qty' => '', 'main_amount' => 0];
				  ...
				  $transfer_recieved_qty_carton_display_previous += ($qty / $pcsPerCarton);
                    ...
                    $total_transfer_recieved_amount_previous+=(float)$transfer_received_amount_previous;
                        ...
                        $get_sales_previous = $preparedSalesDataPrevious[$item->product_id][$item->flavour_id] ?? ['main_qty' => '', 'main_amount' => 0];
                        ...
                        $get_opening = $preparedData[$item->product_id][$item->flavour_id] ?? ['main_qty' => '', 'main_amount' => 0];
                    ...
                    $get_purchase = $preparedPurchaseData[$item->product_id][$item->flavour_id] ?? ['main_qty' => '', 'main_amount' => 0];
                    
                    $purchase_qty = $get_purchase['main_qty'];
                    $purchase_amount = $get_purchase['main_amount'];

                    $get_sales_return = $preparedSalesReturnData[$item->product_id][$item->flavour_id] ?? ['main_qty' => '', 'main_amount' => 0];
                    
                    $sales_return_qty = $get_sales_return['main_qty'];
                    $return_amount = $get_sales_return['main_amount'];

                    $get_transfer_received = $preparedTransferRecievedData[$item->product_id][$item->flavour_id] ?? ['main_qty' => '', 'main_amount' => 0];
                    ...
                    //$get_unpacking_in = $preparedUnpackingInData[$item->product_id][$item->flavour_id] ?? ['main_qty' => '', 'main_amount' => 0];
                    ...
                    //$get_available = $preparedAvailableData[$item->product_id][$item->flavour_id] ?? ['main_qty' => '', 'main_amount' => 0];
                    
                    //$available_qty = $get_available['main_qty'];
                    //$available_amount = $get_available['main_amount'];

                    $get_transfer = $preparedTransferData[$item->product_id][$item->flavour_id] ?? ['main_qty' => '', 'main_amount' => 0];
                    ...
                    $get_sales = $preparedSalesData[$item->product_id][$item->flavour_id] ?? ['main_qty' => '', 'main_amount' => 0];
                    
                    $sales_qty = $get_sales['main_qty'];
                    $sales_amount = $get_sales['main_amount'];


                    //$get_unpacking_out = $preparedUnpackingOutData[$item->product_id][$item->flavour_id] ?? ['main_qty' => '', 'main_amount' => 0];
...
<!-- new code  -->
                    @php
 $closing_qty_display_new = ($available_qty_display) - ($sales_qty_display - $return_qty_display);
    $closing_qty_carton_display_new = ($available_qty_carton_display) - ($sales_qty_carton_display - $return_qty_carton_display);
    $closing_amount_new = ($available_amount) - ($sales_amount - $return_amount);
 ...
 $closing_amount_new = 0;
    ...
    $total_closing_qty_new += ($available_qty_display) - ($sales_qty_display - $return_qty_display);
    $total_closing_carton_qty_new += ($available_qty_carton_display) - ($sales_qty_carton_display - $return_qty_carton_display);
    $total_closing_amount_new += ($available_amount) - ($sales_amount - $return_amount);
