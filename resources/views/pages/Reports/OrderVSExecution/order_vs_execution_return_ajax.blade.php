...
@php
 ...
 @endphp
    @foreach($data as $key => $row)
        ...
        $total_qty_return += $row->total_return_qty;
        $total_amount_return += $row->total_return_amount;
        @endphp
        <tr>
            <td>{{ ++$key }}</td>
...
<tr style="background-color: lightgray;font-size: large;font-weight: bold" >
    ...
    <td>{{ number_format($total_qty_return,2) }}</td>
    <td>{{ number_format($total_amount_return,2) }}</td>
</tr>
</tbody>
</table>
</div>
