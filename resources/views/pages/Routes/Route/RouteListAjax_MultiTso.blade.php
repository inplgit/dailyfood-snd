...
@foreach ($master->get_route_distribuor_wise() as $key => $row)
    <tr>
        ...
        @php
    $tsoNames = [];
    foreach($row->routeTso as $routeTso) {
        if(isset($routeTso->tso->name)) {
            $tsoNames[] = $routeTso->tso->name;
        }
    }
    echo implode(', ', $tsoNames);
@endphp
        ...
        <td class="export-hidden">
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu_sale_order_list" aria-labelledby="dropdownMenuButton">
                    @can('Route_Edit')
                        <a href="{{ route('AddRouteMultiTso_edit', $row->id) }}" class="dropdown-item_sale_order_list dropdown-item">Edit</a>
                    @endcan
                    @can('Route_Delete')
