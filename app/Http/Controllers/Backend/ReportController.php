...
class ReportController extends Controller
{
    ...
    public function order_booker_daily_activity_location_report(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            //     return $row;
            // });
            $saleOrdersAll = SaleOrder::with([
                'usersLocation' => function ($q) {
                    $q->latest();
                }
            ])
                ->whereBetween('dc_date', [$from, $to])
    ...
    public function closing_stock_minus_zero_sub(Request $request)
    {
        ...
        foreach ($result as $item) {
            ...
            foreach (explode(',', $sales_return_qty) as $val) {
                ...
                $unit = isset($qty_explode[1]) ? $qty_explode[1] : '-';
                $pcsPerCarton = isset($qty_explode[2]) ? (float) $qty_explode[2] : 1;
                if ($pcsPerCarton == 0 || $pcsPerCarton == '') {
                    $pcsPerCarton = 1;
                }
            ...
            $tsos = TSO::status()->active()
                ...
                ->when($request->distributor_id != null && $request->tso_id == null, function ($query) use ($request) {
                    ...
                    $query->where('tso.city', $request->city);
                        ...
                        'distributor',
                        'cities'
                    ])->select('tso.*', 'users_distributors.distributor_id')->get()->toArray();
    ...
    public function order_booker_productive_report(Request $request)
    {
        ...
        if ($request->ajax()):
            ...
            $productivity->map(function ($item, $index) use ($productivity) {

                /*
                |--------------------------------------------------------------------------
                | 1. TIME SPENT (inside shop)
                |--------------------------------------------------------------------------
                */
                $item->time_spent_sec = 0;
                $item->time_spent_min = 0;
                $item->time_spent = '-';

                if ($item->time_in && $item->time_out) {
                    $start = \Carbon\Carbon::parse($item->time_in);
                    $end = \Carbon\Carbon::parse($item->time_out);

                    $diff = $end->diffInSeconds($start);
                    $item->time_spent_sec = $diff;
                    $item->time_spent_min = floor($diff / 60);
                    $item->time_spent = gmdate('H:i:s', $diff);
                }

                /*
                |--------------------------------------------------------------------------
                | 2. TIME DIFFERENCE (travel time between shops)
                |--------------------------------------------------------------------------
                */
                $item->time_diff = '-';
                $item->time_diff_sec = 0;
                ...
                if ($index > 0) {
                    ...
                    if ($previous->employee_name == $item->employee_name && $previous->dated == $item->dated) {
                        if ($previous->time_out && $item->time_in) {
                            $prevOut = \Carbon\Carbon::parse($previous->time_out);
                            $currentIn = \Carbon\Carbon::parse($item->time_in);

                            $diff = $currentIn->diffInSeconds($prevOut);

                            $item->time_diff_sec = $diff;
                            $item->time_diff = gmdate('H:i:s', $diff);
                        }
                    }
                }
                ...
                /*
                |--------------------------------------------------------------------------
                | 3. DISTANCE (between previous shop & current shop)
                |--------------------------------------------------------------------------
                */
                $item->distance = 0;
            ...
            $totalNet = $productivity->sum('net_amount');
